...
# 後始末メソッド　app.teardown_appcontext
...
def employer(employer_id):
  ...
  reviews = execute_sql('SELECT job.id, job.title, job.description, job.salary FROM job JOIN employer ON employer.id = job.employer_id WHERE employer.id = ?', employer_id)
  return render_template('employer.html', employer=employer, reviews=reviews)
